...
#define IS_AX(sh)               ((sh->sh_flags & SHF_ALLOC) && (sh->sh_flags & SHF_EXECINSTR))
#define IS_AW(sh)               ((sh->sh_flags & SHF_ALLOC) && (sh->sh_flags & SHF_WRITE))
...
// unsigned int dlmodule_relocate(struct rt_dlmodule *module, Elf_Rel *rel, Elf_Addr sym_val);
...
#endif
Elf32_Addr dlmodule_relocate(rt_uint8_t rel_type, Elf_Addr *where, Elf_Addr sym_val);
