...
#define READ_PROG_HDR(fd, elf_hdr, idx, buf) { \
    lseek(fd, elf_hdr->e_phoff + sizeof(Elf_Phdr) * idx, SEEK_SET); \
    if (sizeof(Elf_Phdr) != (rt_uint32_t)read(fd, buf, sizeof(Elf_Phdr))) \
        BREAK_WITH_WARN(RT_EIO, "read prog_hdr err"); \
}

#define READ_SECT_HDR(fd, elf_hdr, idx, buf) { \
    lseek(fd, elf_hdr->e_shoff + sizeof(Elf_Shdr) * idx, SEEK_SET); \
    if (sizeof(Elf_Shdr) != (rt_uint32_t)read(fd, buf, sizeof(Elf_Shdr))) \
...
rt_err_t dlmodule_load_reltab(struct rt_dlmodule *module, Elf_Rel *reltab, rt_uint32_t rel_cnt, Elf_Sym *symtab,
    rt_uint8_t *strtab) {
    rt_err_t ret;
    rt_uint32_t idx;
    ...
    for (idx = 0; idx < rel_cnt; idx++) {
        ...
        if (STN_UNDEF == sym_idx) {
            ...
            if (SHN_UNDEF != sym->st_shndx) {
                sym_val = (Elf_Addr)(module->mem_space + \
...
rt_err_t dlmodule_load_shared_object(int fd, Elf_Ehdr *elf_hdr, struct rt_dlmodule *module) {
    rt_err_t ret;
    Elf_Phdr *prog_hdr;
    Elf_Shdr *sect_hdr;
    ...
    do {
        ...
        /* alloc prog_hdr */
        prog_hdr = (Elf_Phdr *)rt_malloc(sizeof(Elf_Phdr));
        ...
        for (idx = 0; idx < elf_hdr->e_phnum; idx++) {
            ...
            if (PT_LOAD == prog_hdr->p_type) {
                ...
                dyntab = (Elf_Dyn *)(module->mem_space + \
                    (prog_hdr->p_vaddr - module->vstart_addr));
                dyn_num = prog_hdr->p_filesz / sizeof(Elf_Dyn);
            }
        }
        if (RT_EOK != ret) break;
        ...
        if (dyn_num) {
            Elf_Rel *plttab = RT_NULL;
            Elf_Rel *reltab = RT_NULL;
            ...
            for (idx = 0; idx < dyn_num; idx++) {
                ...
                case DT_PLTREL:
                    ...
                    break;
                case DT_PLTRELSZ:
                    plt_cnt = dyntab[idx].d_un.d_val / sizeof(Elf_Rel);
                ...
                case DT_REL:
                    ...
                    break;
                case DT_RELSZ:
                    rel_cnt = dyntab[idx].d_un.d_val / sizeof(Elf_Rel);
                    LOG_D("rel_cnt %d", rel_cnt);
                    break;
                case DT_RELENT:
                    if (sizeof(Elf_Rel) != dyntab[idx].d_un.d_val)
                ...
                case DT_SYMTAB:
                    ...
                    break;
                case DT_SYMENT:
                    if (sizeof(Elf_Sym) != dyntab[idx].d_un.d_val)
            ...
            /* step 4: get sym_cnt */
            /* alloc sect_hdr */
            sect_hdr = (Elf_Shdr *)rt_malloc(sizeof(Elf_Shdr));
            ...
            for (idx = 0; idx < elf_hdr->e_shnum; idx++) {
                ...
                if (IS_DYNSYM(sect_hdr)) {
                    sym_cnt = sect_hdr->sh_size / sizeof(Elf_Sym);
                    break;
                }
            }
            if (RT_EOK != ret) break;
...
rt_err_t dlmodule_load_relocated_object(int fd, Elf_Ehdr *elf_hdr, struct rt_dlmodule *module) {
    rt_err_t ret;
    Elf_Shdr *sect_hdr;
    rt_uint8_t *shstrab;
    Elf_Rel *reltab;
    ...
    do {
        ...
        for (idx = 0; idx < elf_hdr->e_shnum; idx++) {
            ...
            rel_num = (rt_uint32_t)(sect_hdr->sh_size / sizeof(Elf_Rel));
            LOG_D("rel_num %d", rel_num);

            /* allo reltab */
            LOG_D("reltab sz %d", sect_hdr->sh_size);
            reltab = (Elf_Rel *)rt_malloc(sect_hdr->sh_size);
            ...
            /* alloc symtab */
            symtab = (Elf_Sym *)rt_malloc(sect_hdr->sh_size);
            ...
            for (i = 0; i < rel_num; i++) {
                Elf_Addr addr;
                Elf_Sym *sym = &symtab[ELF32_R_SYM(reltab[i].r_info)];
                LOG_D("sym %s shndx %d", strtab + sym->st_name, sym->st_shndx);

                addr = 0;
                if (STT_FUNC == ELF_ST_TYPE(sym->st_info)) {
                    addr = (Elf_Addr)((rt_uint8_t *)module->mem_space - \
                ...
                if (addr) {
                    Elf_Addr *where = module->mem_space + \
                        (reltab[idx].r_offset - module->vstart_addr);
