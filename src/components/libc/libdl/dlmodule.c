...
static struct rt_module_symtab _rt_module_symtab[] = {
    ADD_MODULE_SYM(dlmodule_find, rt_dlmodule_t, const char *name)
    #include "mo_sym.h"
};
# undef ADD_MODULE_SYM
...
static void _dlmodule_set_name(rt_dlmodule_t *module, const char *path) {
    int size;
...
static void _dlmodule_thread_entry(void* parameter) {
    ...
    do {
        module = (rt_dlmodule_t *)parameter;
...
rt_dlmodule_t *dlmodule_create(void) {
    ...
    module = (rt_dlmodule_t *)rt_object_allocate(
        ...
        for (node = module->object_list.next;
            ...
            if (object->type & RT_Object_Class_Static) {
                ...
                case RT_Object_Class_Module:
                    dlmodule_destroy((rt_dlmodule_t *)object);
                    break;
                default:
...
rt_dlmodule_t *dlmodule_self(void) {
    ...
    tid = rt_thread_self();
    if (tid) {
        ret = (rt_dlmodule_t*) tid->module_id;
    }
 ...
 */
rt_dlmodule_t *rt_module_self(void) {
    return dlmodule_self();
}

rt_dlmodule_t *dlmodule_load(const char* filename) {
    rt_err_t ret;
    int fd;
    Elf32_Ehdr *elf_hdr;
    rt_dlmodule_t *module;
...
rt_dlmodule_t *dlmodule_exec(const char* pgname, const char* cmd,
    int cmd_size) {
    rt_dlmodule_t *module = RT_NULL;
    (void)cmd_size;
...
rt_dlmodule_t *dlmodule_find(const char *name) {
    rt_object_t object;
    rt_dlmodule_t *ret = RT_NULL;

    object = rt_object_find(name, RT_Object_Class_Module);
    if (object) ret = (rt_dlmodule_t *)object;
